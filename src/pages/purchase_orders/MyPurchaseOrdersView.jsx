...
import { getPurchaseOrders, syncPurchaseOrder } from "../../services/myPurchaseOrderService.js";
import { Modal } from "../../components/ui/Modal.jsx";
import { useModals } from "../../hooks/useModals.js";

export default function MyPurchaseOrdersView() {
  ...
  const {
    ...
    totalPages,
    handleSort,
  } = useTableLogic(purchaseOrders, tableConfig);
  ...
  const handleSync = async (order) => {
    openConfirm({
      title: "Sincronizar Orden de Compra",
      msg: `¿Deseas sincronizar la OC <b>${order.id}</b> con SAP?`,
      actionLabel: "Sincronizar",
      variant: "primary",
      onConfirm: async () => {
        const response = await syncPurchaseOrder(order.id);
  ...
  return (
    <div className="space-y-6 justify-center mx-auto lg:px-8">
      ...
      <div>
        ...
        <p className=" /70">
          Revisa el estado de tus órdenes y su integración con SAP.
        </p>
      </div>
      ...
      <GenericFilters
        ...
        filterLabel="Estado de la Orden"
        resultsCount={filteredData.length}
        showAddButton={true}
        addButtonLabel="Nueva Orden"
        onAdd={() => navigate("/dashboard/my-purchase-orders/create")}
      />
      ...
      <GenericTable
        ...
        data={filteredData}
        pageData={pageData}
        emptyMessage="No existen órdenes registradas aún"
        emptyIcon={FileText}
        searchQuery={q}
        onClearSearch={() => setQ("")}
        sortBy={sortBy}
