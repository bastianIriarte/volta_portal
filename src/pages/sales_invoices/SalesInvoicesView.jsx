...
import { getSalesInvoices } from "../../services/SAP/salesInvoiceService.js";
...
export default function SalesInvoicesView() {
  ...
  const {
    ...
    totalPages,
    totalItems,
  } = useServerPagination(getSalesInvoices, {
    defaultSort: "DocDate",
    defaultSortDir: "desc",
    pageSize: 10,
    autoFetch: true,
 ...
 const renderRow = (order, index) => {
    ...
    return (
      <tr key={order.DocEntry || index} className="border-t hover:bg-gray-50">
        ...
        <td className="px-3 py-2 text-xs text-wrap text-left">{order.NumAtCard || "-"}</td>
        <td className="px-3 py-2 text-xs text-wrap">
          <div>
        ...
        <td className="px-3 py-2 text-xs text-center">{formatDate(order.DocDate)}</td>
        <td className="px-3 py-2 text-xs text-center">{formatDate(order.DocDueDate)}</td>
        <td className="px-3 py-2 text-xs text-center font-medium text-wrap">{order.DocTotalSys > 0 ? order.DocTotalSys : order.DocTotal} {order.DocCurrency}</td>
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className=" /70">
          Visualización de facturas de venta (solo lectura)
        </p>
      </div>

      {/* Filtros */}
      <GenericFilters
        searchPlaceholder="Buscar facturas de ventas  (DocNum, DocEntry, Proveedor, Referencia)..."
        searchValue={search}
      ...
      <GenericTable
        ...
        data={invoices}
        pageData={invoices}
        emptyMessage="No hay facturas de ventas registradas"
        emptyIcon={FileText}
        searchQuery={search}
